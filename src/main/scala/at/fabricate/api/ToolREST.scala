...
object ToolREST extends RestHelper {
  // Serve /api/item and friends
  serve( "api" / "tool" prefix {
     
    // /api/item returns all the items
    case Nil JsonGet _ => Tool.findAll : JValue
    
    // /api/item/count gets the item count
    case "count" :: Nil JsonGet _ => JInt(Tool.count)
    
    // /api/item/item_id gets the specified item (or a 404)
    case Tool(tool) :: Nil JsonGet _ => tool: JValue
    
    // DELETE the item in question
    case Tool(tool) :: Nil JsonDelete _ =>
//      if (tool.delete_! ) "OK"
//      else "ERROR"
      if (tool.delete_! ) ("Status" -> "OK") : JValue
      else ("Status" -> "ERROR") : JValue
      //Tool.delete(tool.id).map(a => a: JValue) 
      
    // PUT adds the item if the JSON is parsable
    case Nil JsonPut Tool(tool) -> _ => tool.save: JValue
