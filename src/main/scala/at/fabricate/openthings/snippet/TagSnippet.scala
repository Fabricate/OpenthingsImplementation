...
object TagSnippet extends BaseEntityWithTitleAndDescriptionSnippet[Project] with BaseEntityWithTitleDescriptionIconAndCommonFieldsSnippet[Project] {

      
      val contentLanguage = UrlLocalizer.contentLocale
    
    val tagParam = "tagID"
    object tag extends RequestVar( S.param(tagParam) match {
    ...
    tagParam ->(S.param(tagParam) openOr(""  ))
	    ))

   
    override val TheItem = Project
  override def itemBaseUrl = "tag"
  def tagTemplate = "globalTag"
  def tagTitle = "Tag"
  ...
  val notAvailable = Text("Not avaliable here!")
    ...
    override def getMenu : List[Menu] = 
     List[Menu](
               Menu.i(tagTitle) / tagTemplate  >> Hidden
     )
      // generate the url rewrites
  override def generateRewrites : PartialFunction[RewriteRequest,RewriteResponse] =  {
      case RewriteRequest(ParsePath(List(MatchItemPath(itemBasePath), AsLong(tagID)), _, _, _), _, _) =>
        RewriteResponse(tagTemplate :: Nil, Map("tagID" -> urlDecode(tagID.toString)))
      case RewriteRequest(ParsePath(List(MatchItemPath(itemBasePath)), _, _, _), _, _) =>
        RewriteResponse(tagTemplate :: Nil)
...
*/
  ...
  */
  ...
  def queryItems[T <: BaseEntityWithTitleDescriptionIconAndCommonFields[T]](itemToQuery: BaseMetaEntityWithTitleDescriptionIconAndCommonFields[T] with BaseEntityWithTitleDescriptionIconAndCommonFields[T], otherQueryParams : List[QueryParam[T]] = List() ) = { // , otherQueryParams = List(StartAt(curPage*itemsPerPage), MaxRows(itemsPerPage))
    ...
    val query =
        In(itemToQuery.primaryKeyField,itemToQuery.getTagMapper.taggedItem,By(itemToQuery.getTagMapper.theTag,tag.get)) ::
